...
class ActorCriticAgent:
    ...
    def _loss_critic(
        ...
        """MSE TD loss: (Q(s,a) − [r + γ·V(s')])²  where V(s') = Σ_a π(a|s')·Q(s',a).""" # Is it correct?
        ...
        with torch.no_grad():
            ...
            v_next = (next_action_probs * q_all_next_s_a).sum(dim=-1) # Expected-SARSA target V(s') = Σ_a π(a|s')·Q(s',a).
    ...
    def _entropy_loss(self, states: torch.Tensor) -> torch.Tensor:
        """Negative entropy of π(·|s); minimise to encourage exploration.""" # Has not been checked yet.
        logits = self.network.get_action(states)
